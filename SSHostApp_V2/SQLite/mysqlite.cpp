...
bool MySQLite::setNetWorkIP(QSqlDatabase db, int netNum, int able, QString desIP, QString hostIP, QString port)
{
    QString sqlQuery = "update CONFNET set DESIP = '"+desIP+"',LOCALIP = '"+hostIP+"',PORT = '"+port+"', ABLE = "+QString::number(able)+" where NETNUM = "+QString::number(netNum)+";";
...
QList<QStringList> MySQLite::getRecordList(QSqlDatabase db, QString queryStr)
{
    ...
    if (query.exec(queryStr)) {
        while (query.next()) {
            ...
            stringList.append(query.value(2).toString());//pass
            stringList.append(query.value(3).toString());//ptype
            stringList.append(query.value(4).toString());//pstate
            stringList.append(query.value(5).toString());//value
            stringList.append(query.value(6).toString());//time
            stringList.append(query.value(7).toString());//area
