...
static int flexcan_restart(struct net_device *dev)
{
	//int timeout = D_CAN_WAIT_COUNT;
    struct flexcan_priv *priv = netdev_priv(dev);
    int ret;
	close_candev(dev);
	/*timeout = D_CAN_WAIT_COUNT;
	while((((ret = d_can_read(priv,D_CAN_ES)) & 0xf) != 0x7) && timeout--) {
		udelay(1);
	}
	while((((ret = d_can_read(priv,D_CAN_ES)) & 0xf) != 0x5) && timeout--) {
		udelay(1);
	}*/
	udelay(100);
	open_candev(dev);
		     ...
		     struct can_frame *cf, enum can_state new_state)
{
	...
	case CAN_STATE_BUS_OFF:
		...
		can_bus_off(dev);
		netif_stop_queue(dev);//machao
		close_candev(dev);
		break;
	case CAN_STATE_ERROR_WARNING:	/* fallthrough *///machao
		flexcan_restart(dev);
		open_candev(dev);
		priv->can.state = CAN_STATE_ERROR_ACTIVE;
		break;
	case CAN_STATE_ERROR_PASSIVE:	/* fallthrough *///machao
		flexcan_restart(dev);
		open_candev(dev);
		priv->can.state = CAN_STATE_ERROR_ACTIVE;
		break;
