...
void ConfNode::slotTBtnIndex(int index)
{
    ...
    case 0:
        ...
        MsgBox::showInformation(NULL,tr("操作提示"),tr("设置漏电阈值命令已发送!"),tr("关闭"));
        break;
    case 1:
        //发送查询温度阈值命令
        InitCanPort::sendCmdDate(loop,canId,SCMD_QALARM,N_TEMP,0);
        MsgBox::showInformation(NULL,tr("操作提示"),tr("设置温度阈值命令已发送!"),tr("关闭"));
        break;
    case 2:
        //发送查询固有值命令
        InitCanPort::sendCmdDate(loop,canId,SCMD_QHAVE,N_LEAK,0);
        MsgBox::showInformation(NULL,tr("操作提示"),tr("设置漏电固有值命令已发送!"),tr("关闭"));
        break;
    case 3:
        //发送查询变比值命令
        qDebug()<<"****************";
    ...
    case 4:
        ...
        if (ui->lineEditSetLeak->text().isEmpty()) {
            MsgBox::showInformation(NULL,tr("错误提示"),tr("漏电阈值不能为空!"),tr("关闭"));
            return;
        }
    ...
    case 5:
        ...
        if ( ui->lineEditSetTemp->text().isEmpty()) {
            MsgBox::showInformation(NULL,tr("错误提示"),tr("温度阈值不能为空!"),tr("关闭"));
            return;
        }
    ...
    case 6:
        ...
        if (ui->lineEditSetBase->text().isEmpty()) {
            MsgBox::showInformation(NULL,tr("错误提示"),tr("漏电固有值不能为空!"),tr("关闭"));
            return;
        }
        if (ui->lineEditSetLeak->text().toUInt() > 1500) {
    ...
    case 7:
        ...
        if (ui->lineEditSetRatio->text().isEmpty()) {
            MsgBox::showInformation(NULL,tr("错误提示"),tr("电流变比值不能为空!"),tr("关闭"));
            return;
        }
        if (ui->lineEditSetRatio->text().toUInt() == 0) {
