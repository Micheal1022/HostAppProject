    ...
    ui(new Ui::ConfDevice)
{
    ...
    m_unSelectStr = "QToolButton{font: 12pt '文泉驿等宽微米黑';border:2px solid rgb(0, 122, 165);"
                    "border-radius:10px;color:rgb(0, 122, 165);background-color: rgb(255, 255, 255);}";
    m_selectStr   = "QToolButton{font: 12pt '文泉驿等宽微米黑';border:2px solid rgb(0, 122, 165);"
                    "border-radius:10px;color:rgb(255, 255, 255);background-color: rgb(0, 122, 165);}";

    setGraphicsEffect(new QGraphicsDropShadowEffect,ui->tBtnZH);
    setGraphicsEffect(new QGraphicsDropShadowEffect,ui->tBtnEN);
    ...
    setGraphicsEffect(new QGraphicsDropShadowEffect,ui->tBtnUpdateConf);
    setGraphicsEffect(new QGraphicsDropShadowEffect,ui->tBtnUpdateHost);
    connect(ui->tBtnZH,SIGNAL(clicked(bool)),this,SLOT(slotBtnZH()));
    connect(ui->tBtnEN,SIGNAL(clicked(bool)),this,SLOT(slotBtnEN()));
    ...
    connect(ui->tBtnUpdateHost,SIGNAL(clicked(bool)),this,SLOT(slotBtnUpdateHost()));
    connect(ui->tBtnUpdateConf,SIGNAL(clicked(bool)),this,SLOT(slotBtnUpdateConf()));
    ...
    //ui->gBoxLang->setVisible(false);
    //获取当前语言状态
    QSqlDatabase langdb = MySQLite::openConnection();
    int langType = MySQLite::getLanguageType(langdb);
    MySQLite::closeConnection(langdb);
    if (DEV_ZH == langType) {
        ui->tBtnZH->setStyleSheet(m_selectStr);
        ui->tBtnEN->setStyleSheet(m_unSelectStr);
    } else if (DEV_EN == langType) {
        ui->tBtnZH->setStyleSheet(m_unSelectStr);
        ui->tBtnEN->setStyleSheet(m_selectStr);
    }
...
void ConfDevice::slotBtnUpdateConf()
{
    QFileInfo filePaht_1("/media/sda1/SYSTEM.db");
    if (filePaht_1.exists() == true) {
        ::system("cp /media/sda1/SYSTEM.db /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    QFileInfo filePaht_2("/media/sdb1/SYSTEM.db");
    if (filePaht_2.exists() == true) {
        ::system("cp /media/sdb1/SYSTEM.db /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    QFileInfo filePaht_3("/media/mmcblk0p1/SYSTEM.db");
    if (filePaht_3.exists() == true) {
        ::system("cp /media/mmcblk0p1/SYSTEM.db /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    MsgBox::showInformation(this,tr("操作提示"), tr("文件不存在,更新失败！"),tr("关闭"));
...
void ConfDevice::slotBtnUpdateHost()
{
    QFileInfo filePaht_1("/media/sda1/ssSet");
    if (filePaht_1.exists() == true) {
        ::system("cp /media/sda1/ssSet /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    QFileInfo filePaht_2("/media/sdb1/ssSet");
    if (filePaht_2.exists() == true) {
        ::system("cp /media/sdb1/ssSet /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    QFileInfo filePaht_3("/media/mmcblk0p1/ssSet");
    if (filePaht_3.exists() == true) {
        ::system("cp /media/mmcblk0p1/ssSet /root/SSApp/");
        MsgBox::showInformation(this,tr("操作提示"), tr("更新成功,重启后生效！"),tr("关闭"));
        return;
    }
    MsgBox::showInformation(this,tr("操作提示"), tr("文件不存在,更新失败！"),tr("关闭"));
...
void ConfDevice::slotBtnZH()
{
    QSqlDatabase db = MySQLite::openConnection();
    if (!MySQLite::setLanguageType(db,1)) {
        MsgBox::showInformation(this,tr("操作提示"), tr("设置失败！"),tr("关闭"));
    } else {
        ui->tBtnZH->setStyleSheet(m_selectStr);
        ui->tBtnEN->setStyleSheet(m_unSelectStr);
...
void ConfDevice::slotBtnEN()
{
    QSqlDatabase db = MySQLite::openConnection();
    if (!MySQLite::setLanguageType(db,2)) {
        MsgBox::showInformation(this,tr("操作提示"), tr("设置失败！"),tr("关闭"));
    } else {
        ui->tBtnEN->setStyleSheet(m_selectStr);
        ui->tBtnZH->setStyleSheet(m_unSelectStr);
